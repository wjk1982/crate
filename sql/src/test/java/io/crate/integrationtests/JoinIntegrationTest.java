...
public class JoinIntegrationTest extends SQLTransportIntegrationTest {
    ...
    public void testJoinWithAggregationScalarFunctionWithGroupBy() throws Exception {
        ...
        execute("select fruits.name, max(colors.name), max(fruits.price * fruits.price + 10) + 10, max(fruits.name), count(colors.name) from colors, fruits group by fruits.name order by name");
        assertThat(TestingHelpers.printedTable(response.rows()), is("apple| yellow| 23.60999990940094| apple| 2\nbanana| yellow| 20.640000019073486| banana| 2\n"));
    }
    ...
    public void testDistributedJoinWithAggregationScalarFunctionWithGroupBy() throws Exception {
        ...
        execute("select fruits.name, max(colors.name), max(fruits.price * fruits.price + 10) + 10, max(fruits.name), count(colors.name) from colors, fruits where colors.id = fruits.id group by fruits.name having count(colors.id) > 0 order by name");

        assertThat(TestingHelpers.printedTable(response.rows()), is("apple| red| 23.60999990940094| apple| 1\nbanana| yellow| 20.640000019073486| banana| 1\n"));
    }
}
