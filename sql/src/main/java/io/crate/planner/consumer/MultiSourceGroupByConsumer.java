    ...
    private static class Visitor extends RelationPlanningVisitor {
        ...
        public Plan visitMultiSourceSelect(MultiSourceSelect multiSourceSelect, ConsumerContext context) {
            ...
            if (isExecutedOnHandler(context, plan)) {
                addNonDistributedGroupProtection(plan, splitPoints, groupKeys);
                addFilterProtection(plan, splitPoints, groupKeys, querySpec);
                addTopN(plan, splitPoints, groupKeys, querySpec, context, outputs);
            } else {
                addDistributedGroupProtection(plan, splitPoints, groupKeys);
                plan = createMergePlan(plan, splitPoints, groupKeys, querySpec, context, outputs);
            }
         ...
         */
        private Merge createMergePlan(Plan plan,
            ...
            addFilterProjectionIfNecessary(reducerProjections, querySpec, collectOutputs);
         ...
         */
        private MergePhase createMergePhase(ConsumerContext context, Plan plan, List<Projection> reducerProjections) {
         ...
         */
        private void addNonDistributedGroupProtection(Plan plan,
                                                      SplitPoints splitPoints,
                                                      List<Symbol> groupKeys) {
            ProjectionBuilder projectionBuilder = new ProjectionBuilder(functions);
         ...
         * Adds the group protection to the given plan in order to handle the groupBy.
         */
        private void addDistributedGroupProtection(Plan plan,
                                                   SplitPoints splitPoints,
                                                   List<Symbol> groupKeys) {
            ProjectionBuilder projectionBuilder = new ProjectionBuilder(functions);
            GroupProjection groupProjection = projectionBuilder.groupProjection(
